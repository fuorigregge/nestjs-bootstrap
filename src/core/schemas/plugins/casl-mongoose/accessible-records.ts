...
import type { Schema, Query, Model, Document } from 'mongoose';
...
function accessibleBy<T extends AnyMongoAbility>(
  ...
  if (!modelName) {
    throw new TypeError(
      'Cannot detect model name to return accessible records',
    );
  }

  const query = toMongoQuery(ability, getClass(modelName), action);

  if (query === null) {
    return failedQuery(ability, action || 'read', modelName, this.where());
  }

  return this instanceof mongoose.Query
    ? this.and([query])
    : this.where({ $and: [query] });
}

export interface AccessibleRecordModel<T extends Document, K = {}>
  extends Model<
    T,
    K & {
